"""
Manual entrypoint for the UK RAG ingestion pipeline.

Run from the backend/ directory (needs DATABASE_URL + GEMINI_API_KEY in the env, the
same ones the app uses):

    python -m scripts.ingest_uk                 # ingest UK sources (skips unchanged)
    python -m scripts.ingest_uk --force         # re-embed even if unchanged
    python -m scripts.ingest_uk --review         # list facts awaiting human sign-off
    python -m scripts.ingest_uk --verify <id> --by "Dr Smith"   # sign off one fact

This is intentionally a script, not an API route: initial ingestion and fact sign-off
are operator actions, not user traffic. The scheduled re-crawl is tasks/ingestion.py.
"""
...
async def _run_ingest(force: bool) -> None:
    ...
    try:
        results = await ingest_registry(UK_SOURCES, force=force)
    finally:
        await close_pool()

    print("\n=== UK ingestion summary ===")
    for r in results:
        ...
        print(line)
    print("\nStaged facts are UNVERIFIED. Review them: python -m scripts.ingest_uk --review\n")
...
async def _run_review() -> None:
    ...
    print("\nSign off with: python -m scripts.ingest_uk --verify <id> --by \"Your Name\"\n")
...
def main() -> None:
    p = argparse.ArgumentParser(description="VERA UK RAG ingestion")
    ...
    if args.verify is not None:
        ...
        asyncio.run(_run_ingest(args.force))
